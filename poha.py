...
def format_indian_currency(amount):
    """Format a number into Indian currency style (e.g., ₹1,23,456.78)."""
...
# --- Page Configuration ---
st.set_page_config(
    page_title="Poha Manufacturing Financial Dashboard",
    page_icon="🌾",
    layout="wide"
)
...
def run_financial_model(inputs):
    ...
    # Unpack inputs with error handling
    try:
        hours_per_day = float(inputs.get('hours_per_day', 10))
        days_per_month = float(inputs.get('days_per_month', 24))
        poha_rate_kg_hr = float(inputs.get('poha_rate_kg_hr', 800))
        paddy_yield = float(inputs.get('paddy_yield', 62.0))
        byproduct_sale_percent = float(inputs.get('byproduct_sale_percent', 34.0))
        paddy_rate = float(inputs.get('paddy_rate', 21.5))
        poha_price = float(inputs.get('poha_price', 45.0))
        byproduct_rate_kg = float(inputs.get('byproduct_rate_kg', 5.0))
        land_cost = float(inputs.get('land_cost', 0))
        civil_work_cost = float(inputs.get('civil_work_cost', 0))
        machinery_cost = float(inputs.get('machinery_cost', 7000000))
        machinery_useful_life_years = float(inputs.get('machinery_useful_life_years', 15))
        packaging_cost = float(inputs.get('packaging_cost', 0.5))
        fuel_cost = float(inputs.get('fuel_cost', 0.0))
        other_var_cost = float(inputs.get('other_var_cost', 0.0))
        rent_per_month = float(inputs.get('rent_per_month', 300000))
        labor_per_month = float(inputs.get('labor_per_month', 400000))
        electricity_per_month = float(inputs.get('electricity_per_month', 150000))
        security_ssc_insurance_per_month = float(inputs.get('security_ssc_insurance_per_month', 300000))
        misc_per_month = float(inputs.get('misc_per_month', 300000))
        equity_contrib = float(inputs.get('equity_contrib', 30.0))
        interest_rate = float(inputs.get('interest_rate', 9.0))
        tax_rate_percent = float(inputs.get('tax_rate_percent', 25.0))
        rm_inventory_days = float(inputs.get('rm_inventory_days', 72))
        fg_inventory_days = float(inputs.get('fg_inventory_days', 20))
        debtor_days = float(inputs.get('debtor_days', 45))
        creditor_days = float(inputs.get('creditor_days', 5))
    ...
    if total_capex <= 0:
        return {'error': 'Total Capex must be greater than 0.'}
    if paddy_yield <= 0 or poha_price <= 0:
        return {'error': 'Paddy Yield and Poha Price must be greater than 0.'}
    ...
    daily_paddy_consumption = (poha_rate_kg_hr / (paddy_yield / 100)) * hours_per_day
    ...
    total_annual_depreciation = (machinery_cost + civil_work_cost) / machinery_useful_life_years
    ...
    return {
        'daily_poha_production': daily_poha_production,
        'monthly_poha_production': monthly_poha_production,
        'annual_poha_production': annual_poha_production,
        'daily_paddy_consumption': daily_paddy_consumption,
        'monthly_paddy_consumption': monthly_paddy_consumption,
        'annual_paddy_consumption': annual_paddy_consumption,
        'daily_byproduct_sold': daily_byproduct_sold,
        'monthly_byproduct_sold': monthly_byproduct_sold,
        'annual_byproduct_sold': annual_byproduct_sold,
        ...
        'total_variable_cost_per_kg_poha': total_variable_cost_per_kg_poha,
        'annual_variable_operating_costs_total': annual_variable_operating_costs_total,
        'annual_fixed_operating_costs': annual_fixed_operating_costs,
        'total_operating_expenses_for_pnl': total_operating_expenses_for_pnl,
        'total_annual_depreciation': total_annual_depreciation,
        'operating_income_ebit': operating_income_ebit,
        'rm_inventory_value': rm_inventory_value,
        ...
        'initial_equity_contribution': initial_equity_contribution,
        'total_debt_component': total_debt_component,
        'annual_interest_expense_fixed_assets': annual_interest_expense_fixed_assets,
        'interest_on_working_capital': interest_on_working_capital,
        'annual_interest_expense': annual_interest_expense,
        'earnings_before_tax_ebt': earnings_before_tax_ebt,
        'taxes': taxes,
        ...
        'total_assets_for_roce': total_assets_for_roce,
        'capital_employed': capital_employed,
        **inputs  # Include original inputs in the output
    }

# --- Key Ratio Definitions ---
key_ratios_info = {
    "Net Profit": {"formula": "Net Profit = EBT - Taxes", "explanation": "The final profit after all costs."},
...
def custom_metric(col, label, value, sub_value, info_key):
    """Display a metric with a tooltip in a Streamlit column."""
...
scenarios = {
    "Base Case": {
        ...
        "fuel_cost": 0.0,
        "other_var_cost": 0.0,
        "byproduct_rate_kg": 5.0
    },
    "Best Case": {
        "paddy_rate": 20.0,
        "poha_price": 45.0,
        "paddy_yield": 65.0,
        "interest_rate": 9.0,  # Adjusted to float for consistency
        "equity_contrib": 30.0,
        "packaging_cost": 0.5,
        "fuel_cost": 0.0,
        "other_var_cost": 0.0,
        "byproduct_rate_kg": 6.0
    },
    "Pessimistic Case": {
        "paddy_rate": 25.0,
        "poha_price": 44.0,
...
# Scenario selection
scenario_choice = st.sidebar.selectbox("Select Scenario", list(scenarios.keys()), index=0)

# Initialize session state for scenario parameters
if 'last_scenario' not in st.session_state or st.session_state['last_scenario'] != scenario_choice:
    st.session_state.update(scenarios[scenario_choice])
    st.session_state['last_scenario'] = scenario_choice

# Define input fields with session state persistence
all_inputs = {}
with st.sidebar.expander("Operational Assumptions", expanded=True):
    all_inputs['hours_per_day'] = st.number_input(
        "Production Hours per Day",
        min_value=0, value=st.session_state.get('hours_per_day', 10)
    )
    all_inputs['days_per_month'] = st.number_input(
        "Operational Days per Month",
        min_value=0, value=st.session_state.get('days_per_month', 24)
    )

with st.sidebar.expander("Production & Yield", expanded=True):
    all_inputs['poha_rate_kg_hr'] = st.number_input(
        "Poha Rate (kg/hr)",
        min_value=0, value=st.session_state.get('poha_rate_kg_hr', 800)
    )
    all_inputs['paddy_yield'] = st.number_input(
        "Paddy to Poha Yield (%)",
        min_value=0.0, max_value=100.0, value=st.session_state.get('paddy_yield', 62.0)
    )
    all_inputs['byproduct_sale_percent'] = st.number_input(
        "Byproduct Sold (%)",
        min_value=0.0, max_value=100.0, value=st.session_state.get('byproduct_sale_percent', 34.0)
    )

with st.sidebar.expander("Financial Assumptions (INR)", expanded=True):
    all_inputs['paddy_rate'] = st.number_input(
        "Paddy Rate (INR/kg)",
        min_value=0.0, value=st.session_state.get('paddy_rate', 21.5)
    )
    all_inputs['poha_price'] = st.number_input(
        "Poha Selling Price (INR/kg)",
        min_value=0.0, value=st.session_state.get('poha_price', 45.0)
    )
    all_inputs['byproduct_rate_kg'] = st.number_input(
        "Byproduct Rate (INR/kg)",
        min_value=0.0, value=st.session_state.get('byproduct_rate_kg', 5.0)
    )

with st.sidebar.expander("Capital Expenditure (Capex)", expanded=True):
    all_inputs['land_cost'] = st.number_input(
        "Land Cost",
        min_value=0, value=st.session_state.get('land_cost', 0)
    )
    all_inputs['civil_work_cost'] = st.number_input(
        "Civil Work Cost",
        min_value=0, value=st.session_state.get('civil_work_cost', 0)
    )
    all_inputs['machinery_cost'] = st.number_input(
        "Machinery Cost",
        min_value=0, value=st.session_state.get('machinery_cost', 7000000)
    )
    all_inputs['machinery_useful_life_years'] = st.number_input(
        "Useful Life (Years)",
        min_value=1, value=st.session_state.get('machinery_useful_life_years', 15)
    )

with st.sidebar.expander("Operating Costs", expanded=True):
    st.markdown("##### Granular Variable Costs (per kg)")
    all_inputs['packaging_cost'] = st.number_input(
        "Packaging Material",
        min_value=0.0, value=st.session_state.get('packaging_cost', 0.5)
    )
    all_inputs['fuel_cost'] = st.number_input(
        "Fuel/Power (Variable)",
        min_value=0.0, value=st.session_state.get('fuel_cost', 0.0)
    )
    all_inputs['other_var_cost'] = st.number_input(
        "Other Variable Costs",
        min_value=0.0, value=st.session_state.get('other_var_cost', 0.0)
    )
    st.markdown("##### Detailed Fixed Costs (Monthly)")
    all_inputs['rent_per_month'] = st.number_input(
        "Rent",
        min_value=0, value=st.session_state.get('rent_per_month', 300000)
    )
    all_inputs['labor_per_month'] = st.number_input(
        "Labor Wages and Salaries",
        min_value=0, value=st.session_state.get('labor_per_month', 400000)
    )
    all_inputs['electricity_per_month'] = st.number_input(
        "Electricity (Fixed)",
        min_value=0, value=st.session_state.get('electricity_per_month', 150000)
    )
    all_inputs['security_ssc_insurance_per_month'] = st.number_input(
        "Security, SSC & Insurance",
        min_value=0, value=st.session_state.get('security_ssc_insurance_per_month', 300000)
    )
    all_inputs['misc_per_month'] = st.number_input(
        "Misc Overheads",
        min_value=0, value=st.session_state.get('misc_per_month', 300000)
    )

with st.sidebar.expander("Funding & Tax", expanded=True):
    all_inputs['equity_contrib'] = st.number_input(
        "Equity Contribution (%)",
        min_value=0.0, max_value=100.0, value=st.session_state.get('equity_contrib', 30.0)
    )
    all_inputs['interest_rate'] = st.number_input(
        "Interest Rate (%)",
        min_value=0.0, value=st.session_state.get('interest_rate', 9.0)
    )
    all_inputs['tax_rate_percent'] = st.number_input(
        "Corporate Tax Rate (%)",
        min_value=0.0, max_value=100.0, value=st.session_state.get('tax_rate_percent', 25.0)
    )

with st.sidebar.expander("Working Capital Days", expanded=True):
    all_inputs['rm_inventory_days'] = st.number_input(
        "RM Inventory Days",
        min_value=0, value=st.session_state.get('rm_inventory_days', 72)
    )
    all_inputs['fg_inventory_days'] = st.number_input(
        "FG Inventory Days",
        min_value=0, value=st.session_state.get('fg_inventory_days', 20)
    )
    all_inputs['debtor_days'] = st.number_input(
        "Debtor Days",
        min_value=0, value=st.session_state.get('debtor_days', 45)
    )
    all_inputs['creditor_days'] = st.number_input(
        "Creditor Days",
        min_value=0, value=st.session_state.get('creditor_days', 5)
    )
...
if 'error' in results:
    ...
    custom_metric(col1, "Net Profit", format_indian_currency(results['net_profit']),
                  f"{results['net_profit_margin']:.2f}% Margin", "Net Profit")
    custom_metric(col2, "EBITDA", format_indian_currency(results['ebitda']),
                  f"{results['ebitda_margin']:.2f}% Margin", "EBITDA")
    ...
    summary_data = {
        ...
        "Daily": [f"{results['daily_poha_production']:,.0f}", f"{results['daily_paddy_consumption']:,.0f}",
                  format_indian_currency(results['total_daily_revenue']), format_indian_currency(results['daily_cogs']),
                  format_indian_currency(results['daily_gross_profit'])],
        "Monthly": [f"{results['monthly_poha_production']:,.0f}", f"{results['monthly_paddy_consumption']:,.0f}",
                    format_indian_currency(results['total_monthly_revenue']), format_indian_currency(results['monthly_cogs']),
                    format_indian_currency(results['monthly_gross_profit'])],
        "Annual": [f"{results['annual_poha_production']:,.0f}", f"{results['annual_paddy_consumption']:,.0f}",
    ...
    with col_be2:
        ...
        fig = px.line(be_df, x='Production Volume (kg)', y=['Total Revenue', 'Total Costs'], title="Visual Breakeven Analysis")
    ...
    sensitivity_range = st.slider("Select sensitivity range (% change):", -50, 50, (-20, 20))
    ...
    with col_sens1:
        st.dataframe(sensitivity_df, column_config={
