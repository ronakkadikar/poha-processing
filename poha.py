...
def format_indian_currency(amount):
    """Format a number into Indian currency style (₹) with commas."""
...
def run_financial_model(inputs):
    """Calculate financial metrics based on user inputs, with error handling."""
    # Unpack inputs
...
def update_scenario():
    """Update input values based on selected scenario."""
...
if 'error' in results:
    ...
    for val, res in zip(range_values, sensitivity_results_list):
        if 'error' in res:
            net_profit_str = f"Error: {res['error']}"
        else:
            net_profit_str = format_indian_currency(res['net_profit'])
        sensitivity_data.append({sensitivity_variable: val, 'Net Profit': net_profit_str})

    # Display sensitivity table and plot
    ...
    with col_sens2:
        plot_data = [
            {sensitivity_variable: d[sensitivity_variable], 'Net Profit': res['net_profit']}
            for d, res in zip(sensitivity_data, sensitivity_results_list)
