...
def render_dashboard(inputs):
 ...
 "Amount (INR)": [format_indian_currency(results[k]) if k not in ['annual_cogs', 'annual_fixed_opex', 'annual_var_costs', 'annual_depreciation', 'total_interest', 'taxes'] 
 else f"({format_indian_currency(results[k])})" for k in ['annual_revenue', 'annual_cogs', 'gross_profit', 'annual_fixed_opex', 
 'annual_var_costs', 'annual_depreciation ', 'ebit', 'total_interest', 'ebt', 'taxes', 'net_profit']]
 }
 ...
 "Amount (INR)": [format_indian_currency(results[k]) if k != 'payables' else f"({format_indian_currency(results[k])})" 
 for k in ['total_capex', 'equity', 'debt', 'total_assets', 'rm_inventory', 'fg_inventory', 
 'receivables', 'payables', 'net_working_capital', 'capital_employed']]
 }
